  ...
  Future<String> adminChangeUserPassword(
    ...
    try {
      ...
      if (response.statusCode != 200 && response.statusCode != 204) {
        final message = _serverErrorMessage(
          response,
        ...
        logHandledError(
          'Mise à jour serveur du mot de passe refusée, application locale',
          'POST /api/admin/users/$userId/password → HTTP ${response.statusCode} $message',
        );
      }
    } on AuthException {
      rethrow;
    } catch (e, s) {
      // Mode hors-ligne / cross-origin : application locale immédiate
      logHandledError(
