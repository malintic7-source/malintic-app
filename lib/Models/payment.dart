...
class Payment {
  ...
  factory Payment.fromMap(Map<dynamic, dynamic> data, String id) {
    ...
    return Payment(
      id: id,
      inscriptionId: parseString(data['inscriptionId'] ?? data['inscription_id']),
      ...
      status: parseStatus(parseString(data['status'] ?? data['statut'], defaultValue: 'PaymentStatus.enAttente')),
      methode: parseMethod(parseString(data['methode'] ?? data['method'], defaultValue: 'PaymentMethod.carte')),
      dateCreation: parseDate(data['dateCreation'] ?? data['date_creation'] ?? data['created_at']),
      dateEffectuation: parseDateOrNull(data['dateEffectuation'] ?? data['date_effectuation']),
      referenceTransaction: parseStringOrNull(data['referenceTransaction'] ?? data['reference_transaction'] ?? data['reference']),
      motifEchec: parseStringOrNull(data['motifEchec'] ?? data['motif_echec']),
      motif: parseStringOrNull(data['motif']),
      moduleId: parseStringOrNull(data['moduleId'] ?? data['module_id']),
      trancheNumero: parseInt(data['trancheNumero'] ?? data['tranche_numero'], defaultValue: 1),
      nombreTranches: parseInt(data['nombreTranches'] ?? data['nombre_tranches'], defaultValue: 1),
      remise: parseDouble(data['remise']),
      dateEcheance: parseDateOrNull(data['dateEcheance'] ?? data['date_echeance']),
    );
  }
