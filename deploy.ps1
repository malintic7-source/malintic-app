...
docker compose -p malintic_app build $service $apiService
if ($LASTEXITCODE -ne 0) {
...
docker compose -p malintic_app up -d --no-deps $apiService
if ($LASTEXITCODE -ne 0) {
    throw "La mise à jour de l'API locale a échoué."
}
docker compose -p malintic_app up -d --no-deps $service
if ($LASTEXITCODE -ne 0) {
